...
DEBUG = False
EPSILON = 1e-6
RANDOM_SEED = 224 # Not the Nelson!
MAX_EDGES = 100 # 2000
...
def do2opt_random(N, distances, dist, order, do_longer=False):
    ...
    for cnt in xrange(MAX_R):
        ...
        for _ in xrange(1000):
            if not (order[p2] in exclude or abs(order[p2] - w1) > N_D2):
                break
            p2 = random.randrange(0, N)
        ...
        if delta >= 0:
            if do_longer:
                longer_trips.append((delta, p1, p2, p1a, p2a))
        elif delta < delta_:
            delta_ = delta
        if delta_ < 0:
           break
...
def neighbor_search(N, distances, closest, visited, hash_base, dist, order, update_if_necessary):
    ...
    dist00 = dist
    dist0, order0 = dist, order
    
        
    longer_trips = do2opt_random(N, distances, dist0, order0, do_longer=True)
    
    print 'neighbor_search: longer_trips = %d' % len(longer_trips)
    for delta, p1, p2, p1a, p2a in longer_trips[:5]:
        print '  %.1f (%3d %3d) (%3d %3d)' % (delta, p1, p2, p1a, p2a) 
    ...
    def make_neighbor(dist, order, n, do3):
        found = False
        for _ in range(50):
            changes = make_changes(n)
            
            dist1, order1 = dist, order
            for i in changes:
                delta, p1, p2, p1a, p2a = longer_trips[i % len(longer_trips)]     
                # dist1 won't be accurate as original delta does not apply after re-ordering
                dist1, order1 = do2opt(N, distances, dist1, order1, delta, p1, p2, p1a, p2a)
            
            dist1 = trip2(distances, order1)
            
            normalize(N, order1)            
            #hsh = np.dot(hash_base, order1)
            test = (do3, tuple(changes))
            if not test in tested:
                tested.add(test)
                found = True
                print changes, DIFF(dist1, dist), len(tested)
                break
            ...
            #    break
            #print '_',
        return found, dist1, order1    
        
    while dist > dist00 - EPSILON:
        for do3, MAX_NO_IMPROVEMENT in pass1, pass2:
            ...
            while no_improvement_count <= MAX_NO_IMPROVEMENT:
                ...
                for neighborhood in range(1, MAX_NEIGHBORHOOD):
                    ...
                    actual_dist1 = trip2(distances, order1)
                    assert CLOSE(dist1, actual_dist1), DIFF(dist1, actual_dist1)    
                        
                    # Refine candidate solution using local search and neighborhood
                    dist1, order1 = local_search(N, distances, closest, dist1, order1, False, do3)
                    actual_dist1 = trip2(distances, order1)
                    assert CLOSE(dist1, actual_dist1), DIFF(dist1, actual_dist1) 
                                    
                    assert dist1 > 0
                    if dist1 < dist - EPSILON:
                        print 'Improvement %f => %f' % (dist, dist1)
                        dist, order = dist1, order1
                        normalize(N, order)
                        hsh = np.dot(hash_base, order)
                        update_if_necessary('neighbor_search',  dist, hsh, order)
                        no_improvement_count = 0 # We also restart the search when we find the local optima
