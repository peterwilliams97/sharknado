...
def _do3opt_best(N, distances, dist, order, max_iter):
...
@autojit 
def best_2op(N, distances, dist, order):
    best_d = 0.0
    p1, p2, p1a, p2a = 0,0,0,0
    for p1_ in xrange(N - 4):
        for p2_ in xrange(p1_+2, N - 2):
            delta, _p1, _p2, _p1a, _p2a = calc2opt_delta(N, distances, order, dist, (p1_, p2_))
            if delta < best_d - EPSILON:
                best_d = delta
                p1, p2, p1a, p2a = _p1, _p2, _p1a, _p2a 
    return best_d, ((p1, p2), (p1a, p2a))

    
#@autojit 
def best_3op(N, distances, dist, order):
    best_d, best_deltas = 0.0, None, 
    p1, p2, p3, p1a, p2a, p3a = 0, 0, 0, 0, 0, 0
    for p1_ in xrange(N - 6):
        for p2_ in xrange(p1_+2, N - 4):
            for p3_ in xrange(p2_+2, N - 2):
                boundary_starts = (p1_, p2_, p3_)
                deltas, (p1, p2, p3, p1a, p2a, p3a) = calc3opt_deltas(N, distances, order, dist, boundary_starts)
                for d in deltas:
                    if d < best_d - EPSILON:
                        best_d = d 
                        best_deltas = deltas
    return best_d, best_deltas, (p1, p2, p3, p1a, p2a, p3a) 
    

def do3opt_local(N, distances, dist, order):
    
    #assert len(set(order)) == len(order)
    best_d, best_deltas, best_bundaries3 = 0.0, None,  None
    p1, p2, p1a, p2a = 0,0,0,0
    
    best_d2,((p1, p2), (p1a, p2a)) = best_2op(N, distances, dist, order)
    best_d3, best_deltas, best_bundaries3 = best_3op(N, distances, dist, order)

    dist1, order1 = dist, order
    #print 'best:', best                    
    if best_d2 < best_d3 and best_d2 < 0.0:
        dist1, order1 = do2opt(N, distances, dist, order, best_d, p1, p2, p1a, p2a)
    elif best_d3 < best_d2 and best_d3 < 0.0: # 3-opt    
       
        #print 'best deltas:', deltas
        #imin = min(list(enumerate(deltas)), key=lambda x: x[1])[0] 
        imin = 0
        for i in (1, 2, 3):
            if [i] < best_deltas[imin]:
                imin = i
        assert best_deltas[imin] == best_d3, '%s %d %s' % (best_d3, imin, best_deltas[imin])        
        print '*', (best_d2, best_d3), imin, best_deltas, best_bundaries3
        dist1, order1 = do3_all[imin](N, distances, dist, order, best_deltas, best_bundaries3)
